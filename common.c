...
for the high quality sound edition.  Because they are added at the end, they will not override an explicit setting on the original command line.
...
void *SZ_GetSpace (sizebuf_t *buf, int length)
{
    ...
    if (buf->cursize + length > buf->maxsize)
    {
        if (!buf->allowoverflow)
            Sys_Error ("SZ_GetSpace: overflow without allowoverflow set on cursize %i size %i maxsize %i", buf->cursize,length, buf->maxsize);
...
//	COM_FileBase (path, base);
    Q_strncpy(base, COM_SkipPath(path), sizeof(base));
...
searchpath_t *COM_GetDirSearchPath(searchpath_t *startsearch)
{
    ...
    {
        if (Q_strcmp(search->filename, startsearch->filename))	// different directories, then stop here
        {
            break;
        }
        lastsearch = search;
