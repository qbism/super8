...
#define      HISTORY_FILE_NAME   "quake_history.txt"  //qbism - Baker/ezQuake command history
...
char	*shiftbindings[256];	// Manoel Kasimier - function shift
...
keyname_t keynames[] =
{
	...
	{"MOUSE2", K_MOUSE2},
	{"MOUSE3", K_MOUSE3},
//	{"MOUSE4", K_MOUSE4}, // Manoel Kasimier
//	{"MOUSE5", K_MOUSE5}, // Manoel Kasimier
...
/*				char	*p;
	...
	// previous commands
	if (key == K_UPARROW || key == K_DPAD1_LEFT) // Manoel Kasimier - controller functionality to the console - edited
	{
		do
		{
	...
	if (key == K_DOWNARROW || key == K_DPAD1_RIGHT) // Manoel Kasimier - controller functionality to the console - edited
	{
	...
	// scrolling
	if (key == K_PGUP || key==K_MWHEELUP || key == K_DPAD1_UP) // Manoel Kasimier - controller functionality to the console - edited
	{
		con_backscroll += 2;
	...
	if (key == K_PGDN || key==K_MWHEELDOWN || key == K_DPAD1_DOWN) // Manoel Kasimier - controller functionality to the console - edited
	{
		con_backscroll -= 2;
		if (con_backscroll < 0)
			con_backscroll = 0;
	...
	if (key == K_HOME || key == K_DC_RTRIG) // Manoel Kasimier - controller functionality to the console - edited
	{
		con_backscroll = con_totallines - (vid.height>>3) - 1;
		return;
	}

	if (key == K_END || key == K_DC_LTRIG) // Manoel Kasimier - controller functionality to the console - edited
	{
		con_backscroll = 0;
		return;
	}
...
void Key_Message (int key)
{
	...
	if (key == K_ESCAPE || key == K_DC_B || key == K_DC_START) // Manoel Kasimier - on-screen keyboard - edited
	{
