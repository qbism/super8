...
*/
// vid_win.c -- Win32 video driver -- MGL-less by MH
...
//qbism- Manoel Kasimier - removed  static int		windowed_mouse;
...
cvar_t		vid_fullscreen_mode = {"vid_fullscreen_mode", "8", true};
...
void VID_CreateDIB (int width, int height, unsigned char *palette)
{
	...
	//qbism- only seems to fail on exit, does it matter?
//	if ((previously_selected_GDI_obj = SelectObject (hdcDIBSection, hDIBSection)) == NULL)
//		Sys_Error ("DIB_Init() - SelectObject failed\n");
	previously_selected_GDI_obj = SelectObject (hdcDIBSection, hDIBSection); //qbism
}
...
void VID_InitModes (HINSTANCE hInstance)
{
	...
	wc.lpszMenuName  = 0;
	wc.lpszClassName = "qbismSuper8"; //qbism edited
	...
	modelist[2].width = 854; //qbism
	modelist[2].height = 480;
...
void VID_GetDisplayModes (void)
{
	...
	do
	{
		...
		if ((devmode.dmPelsWidth <= MAXWIDTH) &&
				...
				(devmode.dmPelsWidth >= 512) && //qbism was 640
				(devmode.dmPelsHeight >= 240) && //qbism was 480
...
void	VID_Init (unsigned char *palette)
{
	...
	if (COM_CheckParm("-startwindowed"))  //qbism uncomment
	{
		startwindowed = 1;
...
void VID_Update (vrect_t *rects)
{
	...
	if (firstupdate)
	{
		...
		if ((vid_mode.value != vid_default) &&
				(!startwindowed || (vid_mode.value < MODE_FULLSCREEN_DEFAULT)))
		{
			firstupdate = 0;
			...
			{
				Cvar_SetValue ("vid_mode", windowed_default);
			}
		}
	}
...
// handle the mouse state when windowed if that's changed
	if (modestate == MS_WINDOWED)
	{
//		if (_windowed_mouse != windowed_mouse) // Manoel Kasimier - edited
		{
			if (key_dest != key_menu && !cl.paused) // Manoel Kasimier - edited
			{
				IN_ActivateMouse ();
...
//	M_DrawPic ((320 - p->width) / 2, 4, p);
	M_DrawTransPic ( (320-p->width)/2, 4, p); //qbism from Manoel Kasimier
	...
	for (i = VID_WINDOWED_MODES; i < lnummodes; i++)
	{
		...
		if (ptr && ((pv->width != 360) || 1)) //qbism debug was COM_CheckParm("-allow360")))
		{
			dup = 0;
...
void VID_CheckModedescFixup (int mode)
{
	...
	if (mode == MODE_SETTABLE_WINDOW)
	{
		...
		sprintf (modelist[mode].modedesc, "custom: %dx%d", x, y);
