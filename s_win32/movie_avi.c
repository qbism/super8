...
qboolean Capture_Open (char *filename)
{
    ...
    {
        m_video_frame_size = 4*vid.width * vid.height * 3;
        memset (&bitmap_info_header, 0, sizeof(bitmap_info_header));
        bitmap_info_header.biSize = sizeof(BITMAPINFOHEADER);
        bitmap_info_header.biWidth = vid.width*2;
        bitmap_info_header.biHeight = vid.height*2;
    }
    else
    {
...
void Capture_WriteVideo (byte *pixel_buffer) //qb: call from vid_win.c
{
    ...
    HRESULT	hr;
    if (modelist[vid_modenum].stretched)
        bufsize = 4 * vid.width * vid.height * 3;
    else
